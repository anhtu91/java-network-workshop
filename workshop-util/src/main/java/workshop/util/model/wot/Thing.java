...
public class Thing {
	...
	public JSONObject asThingDescription() {
		...
		this.availableActions.forEach((name, value) -> {
			...
			// link.put("rel", "action");
			link.put("href", String.format("%s/actions/%s", this.hrefPrefix, name));
			links.put(link);
		...
		this.availableEvents.forEach((name, value) -> {
			...
			JSONObject link = new JSONObject();
			// link.put("rel", "event");
			link.put("href", String.format("%s/events/%s", this.hrefPrefix, name));
			links.put(link);
			metadata.put("links", links);
		...
		try {
			...
			obj.put("href", this.hrefPrefix.length() > 0 ? this.hrefPrefix : "/");
			...
			propertiesLink.put("href", String.format("%s/properties", this.hrefPrefix));
			obj.accumulate("links", propertiesLink);

			JSONObject actionsLink = new JSONObject();
			actionsLink.put("rel", "actions");
			actionsLink.put("href", String.format("%s/actions", this.hrefPrefix));
	 ...
	 *
	 * @param actionName
	 *            Optional action name to get descriptions for
	 ...
	 *
	 * @param eventName
	 *            Optional event name to get descriptions for
	 ...
	 */
	public <T> void setProperty(String propertyName, T value) throws IllegalArgumentException {
		Property<T> prop = this.findProperty(propertyName);
		if (prop == null) {
			return;
		}
	 ...
	 *            Name of the event
	 * @param metadata
	 *            Event metadata, i.e. type, description, etc., as a JSONObject
	 */
	...
	public Action performAction(String actionName, JSONObject input) {
		...
		try {
			...
			Action action = (Action) constructor.newInstance(new Object[] { this, input });
	 ...
	 *            Name of the action
	 * @param metadata
	 *            Action metadata, i.e. type, description, etc., as a JSONObject
	 * @param cls
	 *            Class to instantiate for this action
	 */
	public void addAvailableAction(String name, JSONObject metadata, Class cls) {
		if (metadata == null) {
			metadata = new JSONObject();
		}
	...
	// * @param ws The websocket
	// */
	// public void removeSubscriber(WebThingServer.ThingHandler.ThingWebSocket
	// ws) {
	 ...
	 *
	 * @param action
	 *            The action whose status changed
	 */
	...
	private class AvailableEvent {
		...
		// * @param ws The websocket
		// */
		// public void addSubscriber(WebThingServer.ThingHandler.ThingWebSocket
		// ws) {
		// this.subscribers.add(ws);
		// }
		//
		...
		// */
		// public void
		// removeSubscriber(WebThingServer.ThingHandler.ThingWebSocket ws) {
		// if (this.subscribers.contains(ws)) {
		// this.subscribers.remove(ws);
		// }
		// }
		...
		// */
		// public Set<WebThingServer.ThingHandler.ThingWebSocket>
		// getSubscribers() {
	...
	private class AvailableAction {
		 ...
		 *            The action metadata
		 * @param cls
		 *            Class to instantiate for the action
		 */
